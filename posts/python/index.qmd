...
One problem I have about uv is the installation of packages system wide. uv can easily install a Python version

```bash
uv python install
```
Then when I tried to install a package:
```bash
...
This is really not what I want, because I use PyPlot in Julia to call matplotlib, and I do not want an extra virtual environment. Since this is the python version I installed via uv, there is no fear in modiying the system packages. I ended up with

```bash
python -m pip install numpy matplotlib --break-system-packages
```

Alternatively, we should use the `--user` flag. Sadly uv does not plan to support `--user` because they think it's bad for management.
